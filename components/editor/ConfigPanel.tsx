...
export function ConfigPanel({ value, onChange }: { value: PageConfig; onChange: (v: PageConfig) => void }) {
  ...
  return (
    <div className="w-[320px] shrink-0 bg-[var(--dash-card)] border-l border-[var(--dash-border)] h-full overflow-y-auto p-6 space-y-6 shadow-[var(--shadow-card)]">
      <h3 className="text-sm font-semibold tracking-tight text-[var(--dash-text)]" style={{ fontFamily: "Plus Jakarta Sans, system-ui, sans-serif" }}>页面设置</h3>
      ...
      <div>
        ...
        <input type="color" value={value.primaryColor} onChange={e=>set({primaryColor:e.target.value})} className="w-full h-8 rounded-none border border-[var(--dash-border)]" />
      </div>
      ...
      <div>
        ...
        <input type="color" value={value.backgroundColor} onChange={e=>set({backgroundColor:e.target.value})} className="w-full h-8 rounded-none border border-[var(--dash-border)]" />
      </div>
