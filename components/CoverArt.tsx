...
/** 分类几何母题：右侧，带循环微动效（cover-pulse / cover-flow） */
function Motif({ cat, ink, wash, accent, variant, variant4 }: MotifProps) {
  if (cat === "Design") {
    return (
      <>
        ...
        <span
          className="absolute right-[10%] top-[28%] w-[14%] aspect-square cover-pulse"
          style={{ backgroundColor: accent, opacity: variant % 2 === 0 ? 0.4 : 0.28 }}
        />
      </>
    );
  }
  ...
  if (cat === "Typography") {
    return (
      <>
        ...
        <span
          className="absolute right-[18%] top-[52%] w-2 h-2 rounded-full cover-pulse"
  ...
  return (
    <>
      <span className="absolute right-[16%] top-[18%] w-[28%] aspect-square rounded-full border" style={{ borderColor: ink, opacity: 0.2 }} />
      <span className="absolute right-[20%] top-[24%] w-[18%] aspect-square rounded-full border" style={{ borderColor: accent, opacity: 0.4 }} />
      <span className="absolute right-[24%] top-[30%] w-2 h-2 rounded-full cover-pulse" style={{ backgroundColor: accent, opacity: 0.55 }} />
    </>
  );
}
...
/**
 * 共享封面底盘：大衬线首字母 + 分类几何母题 + 底栏编号 + 循环微动效。
 * HeroCover / ArticleArt 均由此封装。
 */
export const CoverArt = memo(function CoverArt({
  ...
  const letterSize = ratio === "wide" ? "text-[6.5rem]" : "text-[5rem]";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 cover-inner">
        ...
        <span
          className={`absolute serif italic leading-none select-none font-light tracking-tighter left-[12%] top-[20%] ${letterSize}`}
