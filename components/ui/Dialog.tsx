...
export function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[var(--dash-card)] rounded-none shadow-[var(--shadow-pop)] border border-[var(--dash-border)] w-full max-w-md p-6 animate-[scaleIn_0.2s_var(--ease-out)]">
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button
            ...
            className={`px-4 py-2 text-sm rounded-none transition-colors font-medium ${variant === "danger" ? "bg-red-600 text-white hover:bg-red-700 border border-red-600" : "bg-[var(--dash-text)] text-white hover:opacity-90 border border-[var(--dash-text)]"}`}
          >
            {confirmText}
          </button>
        </div>
...
export function PromptDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => onOpenChange(false)} />
      <form onSubmit={handleSubmit} className="relative bg-[var(--dash-card)] rounded-none shadow-[var(--shadow-pop)] border border-[var(--dash-border)] w-full max-w-md p-6">
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button type="submit" className="px-4 py-2 text-sm bg-[var(--dash-text)] text-white rounded-none hover:opacity-90 font-medium">确认</button>
        </div>
      </form>
    </div>
  )
