...
export const ArticleArt = memo(function ArticleArt({
  ...
  return (
    ...
    >
      ...
      {/* 微动效：封面元素缓慢浮动 */}
      <style>{`
        .cover-float { animation: coverFloat 6s ease-in-out infinite; }
        .cover-float-delay { animation: coverFloat 6s ease-in-out infinite 2s; }
        .cover-pulse { animation: coverPulse 4s ease-in-out infinite; }
      `}</style>

      {/* Design */}
      {catName === "Design" && variant === 0 && (<><span className={`absolute -top-6 font-serif italic leading-none select-none left-6 cover-float ${tall ? "text-[11rem]" : "text-[8rem]"}`} style={{ color: palette.ink }}>{initial}</span><span className="absolute bottom-4 right-6 block rounded-full h-16 w-16 cover-float-delay" style={{ backgroundColor: palette.wash, border: `2px solid ${palette.ink}` }} /><span className="absolute top-1/2 left-6 right-6 h-px" style={{ backgroundColor: palette.ink }} /></>)}
      ...
      {catName === "Life" && variant === 4 && (<div className="absolute inset-0 flex items-center justify-center"><div className="relative"><div className="w-28 h-28 rounded-full border-[3px] border-dashed flex items-center justify-center cover-pulse" style={{ borderColor: palette.ink }}><div className="w-20 h-20 rounded-full border-2 flex items-center justify-center" style={{ borderColor: palette.ink }}><span className="text-xl" style={{ color: palette.ink }}>☀️</span></div></div><div className="absolute -top-2 -right-2 w-5 h-5 rounded-full" style={{ backgroundColor: palette.ink }} /></div></div>)}
