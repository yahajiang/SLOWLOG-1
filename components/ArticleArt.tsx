...
function FallbackCover({ initial, palette, variant }: { initial: string; palette: any; variant: number }) {
  const F: React.JSX.Element[] = [
    <div key={0} className="absolute inset-0">
      <span className="absolute -top-6 left-6 serif italic leading-none select-none text-[8rem] tracking-tighter" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
      ...
      <span className="absolute bottom-7 left-6 mono text-[11px] tracking-[0.2em]" style={{ color: palette.ink, opacity: 0.35 }}>No. {variant}</span>
    </div>,
    ...
    <div key={3} className="absolute inset-0">
      ...
      <span className="absolute top-10 h-20 w-20 rounded-none mix-blend-multiply left-1/2" style={{ backgroundColor: palette.ink, opacity: 0.28 }} />
      <span className="absolute bottom-5 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.32 }} />
      <span className="absolute bottom-8 left-6 serif italic text-sm" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
    </div>,
    <div key={4} className="absolute inset-0 flex items-center justify-center">
      <span className="serif italic leading-none select-none text-7xl" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
    </div>,
    <div key={5} className="absolute inset-0 flex flex-col items-center justify-center gap-3">
      ...
      <span className="serif italic leading-none select-none text-6xl" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
      <span className="w-10 h-px" style={{ backgroundColor: palette.ink }} />
    </div>,
...
function PluginSymbol({ symbol, palette, variant, variant4, posClass }: { symbol: TagSymbol | null; palette: any; variant: number; variant4?: number; posClass?: string }) {
    ...
    return (
      <span className={`${badgeBase} gap-1.5`}>
        <span className="w-9 h-9 rounded-none border bg-[var(--dash-card)]/90 flex items-center justify-center shadow-sm" style={{ borderColor: c }}>
          <span className="w-3 h-3 rounded-none" style={{ backgroundColor: c, opacity: 0.35 }} />
        </span>
    ...
    return (
      <span className={`${badgeBase} w-11 h-9 border rounded-[6px] bg-[var(--dash-card)]/90 overflow-hidden flex flex-col shadow-sm`} style={style}>
        <span className="h-[10px] border-b flex items-center gap-[3px] px-1.5" style={{ borderColor: c, backgroundColor: w }}>
          <span className="w-1.5 h-1.5 rounded-none" style={{ backgroundColor: c, opacity: 0.35 }} />
          <span className="w-1.5 h-1.5 rounded-none" style={{ backgroundColor: c, opacity: 0.31 }} />
        </span>
        <span className="flex-1 flex">
          <span className="flex-1 border-r" style={{ borderColor: c, opacity: 0.15 }} />
  ...
  if (v === 1) return (<span className={`${badgeBase} w-10 h-10 rounded-none border bg-[var(--dash-card)]/90 flex items-center justify-center shadow-sm`} style={style}><span className="w-4 h-4 rounded-none" style={{ backgroundColor: c, opacity: 0.15 }} /><span className="absolute w-2.5 h-2.5 rounded-none" style={{ backgroundColor: c }} /></span>);
...
function TagScene({ symbol, palette, variant }: { symbol: TagSymbol | null; palette: any; variant: number }) {
    ...
    return (
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          <rect x="44" y="22" width="36" height="36" fill={w} stroke={c} strokeWidth="1.5" />
          <rect x="84" y="22" width="36" height="36" fill="transparent" stroke={c} strokeWidth="1.5" />
          <rect x="44" y="62" width="36" height="36" fill="transparent" stroke={c} strokeWidth="1.5" />
          <rect x="84" y="62" width="36" height="36" fill={c} stroke={c} strokeWidth="1.5" />
          <circle cx="124" cy="98" r="2.5" fill={a} />
          <line x1="22" y1="100" x2="178" y2="100" stroke={c} strokeWidth="0.75" opacity="0.35" />
          <text x="100" y="112" textAnchor="middle" fontSize="8" fill={c} opacity="0.4" fontFamily="JetBrains Mono">2×2 · grid</text>
        </svg>
      </div>
    );
  }
    ...
    return (
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          ...
          <line x1="14" y1="60" x2="56" y2="60" stroke={c} strokeWidth="1.25" opacity="0.55" />
          ...
          <line x1="100" y1="76" x2="100" y2="92" stroke={c} strokeWidth="0.75" opacity="0.35" />
          <line x1="86" y1="92" x2="114" y2="92" stroke={c} strokeWidth="0.75" opacity="0.35" />
        </svg>
      </div>
    );
  }
    ...
    return (
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          ...
          <circle cx="74" cy="60" r="14" fill="none" stroke={c} strokeWidth="0.75" opacity="0.5" />
          <circle cx="126" cy="60" r="14" fill="none" stroke={c} strokeWidth="0.75" opacity="0.5" />
          <text x="100" y="106" textAnchor="middle" fontSize="8" fill={c} opacity="0.4" fontFamily="JetBrains Mono">mirror</text>
        </svg>
      </div>
    );
  }
    ...
    return (
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          ...
          <ellipse cx="100" cy="32" rx="26" ry="4" fill={w} stroke={c} strokeWidth="1.25" />
          {/* 蒸汽 */}
          <path d="M 86 18 Q 90 12 86 6" stroke={c} strokeWidth="1.25" fill="none" opacity={layers === 0 ? 0.6 : 0.3} />
          <path d="M 100 16 Q 104 10 100 4" stroke={c} strokeWidth="1.25" fill="none" opacity={layers === 1 ? 0.6 : 0.3} />
          <path d="M 114 18 Q 118 12 114 6" stroke={c} strokeWidth="1.25" fill="none" opacity={layers === 2 ? 0.6 : 0.3} />
          {/* 层状 */}
          {layers === 0 && (
            <>
          ...
          {layers === 1 && (
            <>
              <path d="M 82 60 Q 100 54 118 60" stroke={c} strokeWidth="1.25" fill="none" />
              <path d="M 82 72 Q 100 66 118 72" stroke={c} strokeWidth="1.25" fill="none" />
              <path d="M 82 84 Q 100 78 118 84" stroke={c} strokeWidth="1.25" fill="none" />
            </>
          )}
          {layers === 2 && (
            <>
    ...
    return (
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          ...
          <text x="100" y="112" textAnchor="middle" fontSize="8" fill={c} opacity="0.4" fontFamily="JetBrains Mono">soulsync</text>
        </svg>
      </div>
    );
  }
    ...
    return (
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          ...
          <rect x="50" y="20" width="100" height="12" fill={w} stroke={c} strokeWidth="1.25" />
          ...
          {/* 内容行 */}
          <line x1="60" y1="42" x2="120" y2="42" stroke={c} strokeWidth="0.75" opacity="0.3" />
          <line x1="60" y1="50" x2="110" y2="50" stroke={c} strokeWidth="0.75" opacity="0.3" />
          <line x1="60" y1="58" x2="130" y2="58" stroke={c} strokeWidth="0.75" opacity="0.3" />
          <line x1="60" y1="66" x2="100" y2="66" stroke={c} strokeWidth="0.75" opacity="0.3" />
          <line x1="60" y1="74" x2="118" y2="74" stroke={c} strokeWidth="0.75" opacity="0.3" />
          {/* 纸出 */}
          <path d="M 96 84 L 96 102 L 110 102 L 110 84" fill="white" stroke={c} strokeWidth="1.25" />
      ...
      <div className={wrap}>
        <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
          {edges.map(([a, b], i) => {
            ...
            return <line key={i} x1={na[0]} y1={na[1]} x2={nb[0]} y2={nb[1]} stroke={c} strokeWidth="0.75" opacity="0.45" />;
          })}
          {nodes.map(([x, y], i) => (
            <circle key={i} cx={x} cy={y} r="3" fill="white" stroke={c} strokeWidth="1.25" />
          ))}
          <polygon points="100,30 130,46 130,76 100,92 70,76 70,46" fill="none" stroke={c} strokeWidth="1.25" opacity="0.35" strokeDasharray="3 3" />
  ...
  return (
    <div className={wrap}>
      <svg viewBox="0 0 200 120" className="w-full h-full" aria-hidden>
        <circle cx="100" cy="60" r="38" fill="none" stroke={c} strokeWidth="0.75" opacity="0.4" strokeDasharray="2 3" />
        <circle cx="100" cy="60" r="24" fill="none" stroke={c} strokeWidth="0.8" opacity="0.55" />
        <circle cx="100" cy="60" r="12" fill={w} stroke={c} strokeWidth="1.25" />
...
export const ArticleArt = memo(function ArticleArt({
  ...
  return (
    ...
    >
      {/* 纸纹 + 微噪点 */}
      <div className="absolute inset-0" style={{ backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 3px, ${palette.wash} 3px, ${palette.wash} 4px)`, opacity: catName === "Design" ? 0.20 : catName === "Plugin" ? 0.16 : 0.15 }} />
      {/* 顶部细发丝线（编辑感） */}
        ...
        <>
          ...
          {variant3 === 0 && (
            <>
              ...
              <span className="absolute bottom-5 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              <span className="absolute bottom-[26px] left-6 mono text-[10px] tracking-[0.2em] select-none" style={{ color: palette.ink, opacity: 0.35 }}>{abbr} · {noNum} — · ED.</span>
          ...
          {variant3 === 1 && (
            <>
              ...
              <span className="absolute bottom-4 right-7 w-12 h-12 rounded-none border translate-x-1 -translate-y-1 opacity-35" style={{ borderColor: palette.ink }} />
          ...
          {variant3 === 2 && (
            <>
              ...
              <span className="absolute top-7 left-6 right-6 flex items-center gap-3">
                <span className="h-px flex-1" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
                <span className="serif italic text-2xl tracking-tight" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
                <span className="mono text-[8px] tracking-[0.2em] border px-2 py-0.5 rounded-none" style={{ borderColor: palette.ink, opacity: 0.28, color: palette.ink }}>{abbr}</span>
                <span className="h-px flex-1" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              </span>
      ...
      {catName === "Plugin" && (
        <>
          <LightTexture palette={palette} intensity="low" />
          <div className="absolute inset-4 rounded-[8px] border opacity-40" style={{ borderColor: palette.wash, backgroundImage: `linear-gradient(${palette.wash} 1px, transparent 1px), linear-gradient(90deg, ${palette.wash} 1px, transparent 1px)`, backgroundSize: `${gridSize} ${gridSize}` }} />
          <span className="absolute top-[18px] left-[18px] mono text-[6px] tracking-[0.12em] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>—</span>
          {variant3 === 0 && (
            <>
              <span className="absolute top-[34%] left-6 right-10 h-px" style={{ backgroundColor: palette.ink, opacity: 0.35 }} />
          ...
          {variant3 === 1 && (
            <>
              <span className="absolute top-[28%] left-6 right-10 h-px" style={{ backgroundColor: palette.ink, opacity: 0.28 }} />
          ...
          {variant3 === 2 && (
            <>
              <span className="absolute top-6 left-6 w-6 h-6 border-l border-t rounded-tl-[6px]" style={{ borderColor: palette.ink, opacity: 0.28 }} />
              ...
              <span className="absolute top-[34%] left-6 right-10 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
            </>
          )}
          <span className="absolute bottom-6 left-6 mono text-[10px] tracking-[0.18em] select-none flex items-center gap-1.5" style={{ color: palette.ink, opacity: 0.35 }}>
        ...
        <>
            ...
            <>
              ...
              <span className="absolute top-7 left-7 right-7 h-[54%] border rounded-[10px] bg-[var(--dash-card)]/85 overflow-hidden shadow-sm" style={{ borderColor: palette.wash }}>
                ...
                <span className="absolute top-0 left-0 right-0 h-[16px] border-b flex items-center gap-1.5 px-2.5" style={{ borderColor: palette.wash, backgroundColor: palette.paper }}>
                  ...
                  <span className={`ml-1 w-1.5 h-1.5 rounded-none ${variant4 === 0 ? "opacity-100" : "opacity-40"}`} style={{ backgroundColor: palette.accent }} />
                </span>
                {/* 3. 内容行 1 */}
                ...
                {/* 6. 状态栏 */}
                <span className="absolute bottom-2 left-3 right-3 h-1.5 rounded-none" style={{ backgroundColor: palette.accent, opacity: 0.35 }} />
                {/* 7. URL 标签 */}
                <span className="absolute top-2 right-2 mono text-[6px] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>localhost:3000</span>
              </span>
              {/* 8. 底栏 */}
              <span className="absolute bottom-6 left-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>FRT · {noNum} ▭ WIN</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
          {variant3 === 1 && (
            <>
              ...
              {/* 3. 状态文字 */}
              <span className="absolute bottom-7 left-7 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>FRT · {noNum} ⊞ GRID</span>
              {/* 4. 4. 5. 装饰点 */}
          ...
          {variant3 === 2 && (
            <>
              ...
              {/* 3. 巨字 */}
              <span className="absolute bottom-8 left-7 serif italic text-2xl tracking-tight" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
              {/* 4. 字号标 */}
              <span className="absolute bottom-7 left-16 mono text-[8px] tracking-[0.15em] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>Aa · {12 + variant4 * 2}px</span>
              {/* 5. 度量小格 */}
              ...
              {/* 6. 底分割线 */}
              <span className="absolute bottom-6 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              {/* 7. 类别小标 */}
              <span className="absolute bottom-[26px] right-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>FRT · {noNum} COMP</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
        </>
        ...
        <>
          ...
          {variant3 === 1 && (
            <>
              <span className="absolute top-6 left-6 right-6 bottom-10 bg-[var(--dash-card)]/90 rounded-[8px] border p-3 shadow-sm" style={{ borderColor: palette.wash }}>
                <span className="flex items-center gap-2 mb-2"><span className="w-3 h-3 rounded border flex items-center justify-center" style={{ borderColor: variant4 <= 1 ? palette.accent : palette.ink, backgroundColor: variant4 <= 1 ? palette.accent : "transparent", opacity: variant4 <= 1 ? 1 : 0.4 }}><span className="text-white text-[8px] leading-none" style={{ opacity: variant4 <= 1 ? 1 : 0 }}>✓</span></span><span className="h-1.5 rounded-none" style={{ backgroundColor: palette.ink, opacity: 0.22, width: `${14 + variant4 * 2}rem` }} /><span className="ml-auto mono text-[7px] tracking-[0.15em] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>{variant4 <= 1 ? "DONE" : "TODO"}</span></span>
          ...
          {variant3 === 2 && (
            <>
              ...
              <span className="absolute top-6 right-10 mono text-[6px] tracking-[0.15em] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>RULED · {variant4}</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
          <span className="absolute bottom-6 left-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>{abbr} · {noNum} — · {variant4}</span>
      ...
      {catName === "Life" && (
        <>
          ...
          {variant3 === 0 && (
            <>
              <span className="absolute -top-6 -right-8 w-32 h-32 rounded-none" style={{ background: `radial-gradient(circle at 40% 40%, ${palette.wash}, transparent 65%)`, opacity: 0.28 + variant4 * 0.05 }} />
              <span className="absolute -bottom-4 -left-6 w-20 h-20 rounded-none" style={{ background: `radial-gradient(circle at 60% 60%, ${palette.accent}, transparent 60%)`, opacity: 0.22 }} />
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 320 180" preserveAspectRatio="none" aria-hidden>
                ...
                <path d="M 10 142 Q 80 102 160 142 T 310 142" fill="none" stroke={palette.wash} strokeWidth="1.25" opacity="0.55" strokeDasharray="3 4" />
              </svg>
              ...
              <span className="absolute bottom-7 left-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>LIFE · {noNum} 〜</span>
              <span className="absolute bottom-6 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
          {variant3 === 1 && (
            <>
              ...
              <span className="absolute bottom-7 left-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>LIFE · {noNum} ~</span>
              <span className="absolute bottom-7 right-6 mono text-[9px] opacity-50" style={{ color: palette.ink, opacity: 0.35 }}>DAILY · {variant4}</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
          {variant3 === 2 && (
            <>
              ...
              <div className="absolute inset-7 grid grid-cols-6 gap-2" style={{ opacity: 0.35 + variant4 * 0.04 }}>
                {Array.from({ length: 12 }).map((_, i) => (
                  <span key={i} className="w-1 h-1 rounded-none" style={{ backgroundColor: (i + variant4) % 3 === 0 ? palette.ink : palette.wash }} />
                ))}
              </div>
              <span className="absolute bottom-9 left-6 serif italic text-2xl tracking-tight" style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
              <span className="absolute bottom-9 left-12 mono text-[8px] tracking-[0.15em] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>LIFE LOG · {variant4}</span>
              <span className="absolute top-3 right-3 w-1.5 h-1.5 rounded-none" style={{ backgroundColor: palette.accent }} />
              <span className="absolute bottom-6 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              <span className="absolute bottom-[26px] right-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>LIFE · {noNum} LOG</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
        </>
        ...
        <>
          ...
          {variant3 === 0 && (
            <>
              ...
              {/* 2. 横基线 */}
              <span className="absolute left-6 right-6 top-[40%] h-px opacity-35" style={{ backgroundColor: palette.ink }} />
              <span className="absolute left-6 right-6 top-[40%] translate-y-[2px] h-px opacity-15" style={{ backgroundColor: palette.ink }} />
              {/* 3. 巨衬线 Aa */}
              <span className={`absolute top-[28%] ${shiftClass} serif italic leading-none select-none text-7xl font-light`} style={{ color: palette.ink, opacity: 0.35 }}>{initial}</span>
              {/* 4. 字号小标 */}
              <span className="absolute top-6 right-6 serif italic text-xs opacity-50" style={{ color: palette.ink, opacity: 0.35 }}>Aa · 56pt</span>
              {/* 5. pt 度量 */}
              <span className="absolute bottom-[34px] left-6 mono text-[10px] tracking-[0.2em] select-none" style={{ color: palette.ink, opacity: 0.35 }}>TYPE · {noNum} · x-height {40 + variant4 * 4}</span>
              {/* 6. 底部细发丝线 */}
          ...
          {variant3 === 1 && (
            <>
              ...
              {/* 3. 引用样本 */}
              <span className="absolute bottom-8 left-16 mono text-[8px] tracking-[0.1em] opacity-40" style={{ color: palette.ink, opacity: 0.35 }}>"The quick brown fox · {variant4}px"</span>
              {/* 4. 度量竖线 */}
              ...
              <span className="absolute bottom-6 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              {/* 7. 类别小标 */}
              <span className="absolute bottom-[26px] right-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>TYP · {noNum} · {variant4}</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
          {variant3 === 2 && (
            <>
              ...
              <div className="absolute top-6 left-6 right-6 bottom-10 grid grid-cols-2 gap-3 opacity-40">
                <span className="border-l-2 pl-2.5 flex flex-col gap-1.5" style={{ borderColor: variant4 % 2 === 0 ? palette.wash : palette.ink }}>
                  <span className="h-1.5 w-full rounded-none" style={{ backgroundColor: palette.ink, opacity: 0.28 }} />
                ...
                <span className="border-l-2 pl-2.5 flex flex-col gap-1.5" style={{ borderColor: palette.wash }}>
                  <span className="h-1.5 w-full rounded-none" style={{ backgroundColor: palette.ink, opacity: 0.15 }} />
                  <span className="h-1 rounded-none" style={{ backgroundColor: palette.wash, width: `${60 + variant4 * 5}%` }} />
                </span>
              </div>
              {/* 2. 栏标签 */}
              <span className="absolute top-9 left-7 mono text-[6px] opacity-50" style={{ color: palette.ink, opacity: 0.35 }}>{12 + variant4} COL</span>
              <span className="absolute top-9 left-[50%] ml-1 mono text-[6px] opacity-50" style={{ color: palette.ink, opacity: 0.35 }}>{8 + variant4} COL</span>
              {/* 3. 大字 Aa */}
              <span className="absolute bottom-7 left-6 serif italic text-2xl" style={{ color: palette.ink, opacity: 0.35 }}>Aa</span>
              {/* 4. 字号对照 */}
              <span className="absolute bottom-7 right-20 mono text-[8px] tracking-[0.15em] opacity-50" style={{ color: palette.ink, opacity: 0.35 }}>·</span>
              {/* 5. 底分割线 */}
              <span className="absolute bottom-6 left-6 right-6 h-px" style={{ backgroundColor: palette.ink, opacity: 0.22 }} />
              {/* 6. 类别小标 */}
              <span className="absolute bottom-6 right-6 mono text-[10px] tracking-[0.18em]" style={{ color: palette.ink, opacity: 0.35 }}>TYP · {noNum} GRID</span>
              <PluginSymbol symbol={symbol as any} palette={palette} variant={variant} variant4={variant4} />
            </>
          )}
        </>
