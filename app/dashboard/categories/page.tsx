...
export default function CategoriesPage(){
  ...
  const confirmDel=async()=>{
    ...
    if(!r.ok){const j=await r.json(); toast(j.error||(lang === "zh" ? "删除失败" : "Delete failed"),"error")} else toast(lang === "zh" ? "已删除" : "Deleted","success")
    setDelId(null)
    load()
  }
        ...
        {cats.map(c=>(
          <div key={c.id} className="flex items-center justify-between p-4 hover:bg-[var(--dash-bg)]">
            <div>
              ...
              <p className="text-xs text-[var(--dash-muted)]">{c.slug} · {c._count?.posts ?? 0} {lang === "zh" ? "篇" : " posts"}</p>
            </div>
