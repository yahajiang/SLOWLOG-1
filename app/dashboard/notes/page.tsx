...
export default function NotesPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <h1 className="text-xl font-semibold tracking-tight text-[var(--dash-text)]" style={{ fontFamily: "Plus Jakarta Sans, system-ui, sans-serif" }}>随想</h1>
      <div className="bg-[var(--dash-card)] border border-[var(--dash-border)] rounded-none p-4 flex gap-3 shadow-[var(--shadow-card)]">
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="写点什么... (≤500字，自动识别链接)" maxLength={500} className="flex-1 px-4 py-2 text-sm border border-[var(--dash-border)] rounded-none bg-[var(--dash-bg)] focus:bg-[var(--dash-card)] focus:border-[var(--dash-accent)] focus:outline-none focus:ring-1 focus:ring-[var(--dash-accent)]/20" />
        <button onClick={submit} disabled={loading||!input.trim()} className="px-6 py-2 bg-[var(--dash-text)] text-white text-sm rounded-none disabled:opacity-50 hover:opacity-90 font-medium">发布</button>
      </div>
      <div className="space-y-3">
        {notes.map(n=>(
          <div key={n.id} className="bg-[var(--dash-card)] border border-[var(--dash-border)] rounded-none p-4 flex justify-between gap-4 shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-float)] transition-shadow">
            ...
            <button onClick={()=>del(n.id)} className="text-xs text-[var(--dash-muted)] hover:text-red-600 hover:bg-red-50 px-2 py-1 rounded-none border border-transparent hover:border-red-200 shrink-0">删除</button>
          </div>
        ))}
