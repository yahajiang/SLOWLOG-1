          ...
          {paged.map((p) => (
            <div key={p.id} className="flex items-center gap-3 p-4 hover:bg-[var(--dash-bg)] group">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[var(--dash-muted)] mt-1 truncate">{p.category?.nameZh || p.category?.name || "未分类"} · <span className={`px-1.5 py-0.5 rounded-none text-[10px] border ${p.status === "published" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : p.status === "draft" ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-[var(--dash-bg)] text-[var(--dash-muted)] border-[var(--dash-border)]"}`}>{p.status}</span> · {new Date(p.createdAt).toLocaleDateString()} · {p.tags?.slice(0,2).join(", ")}</p>
              </div>
